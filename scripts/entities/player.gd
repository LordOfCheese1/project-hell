...
func _physics_process(delta):
	...
	for i in range(len(grabbed_items)):
		grabbed_items[i].position = lerp(grabbed_items[i].position, Vector2(position.x + (look_dir_x * 16) + grabbed_items.find(grabbed_items[i]) * look_dir_x * -32, position.y + (look_dir_y * 16)), 0.3)
